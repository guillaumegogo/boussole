...
function update_offre($id_offre, $nom, $desc, $date_debut, $date_fin, $sous_theme, $adresse, $code_postal, $ville, $courriel, $tel, $url, $delai, $zone, $actif, $user_id)
{
	...
	$code_insee = '';
	$query = 'SELECT code_insee FROM `'.DB_PREFIX.'bsl__ville` 
	...
	$stmt2 = mysqli_prepare($conn, $req);
	mysqli_stmt_bind_param($stmt2, 'ssssisssssssiiiii', $nom, $desc, date('Y-m-d', strtotime(str_replace('/', '-', $date_debut))), date('Y-m-d', strtotime(str_replace('/', '-', $date_fin))), $sous_theme, $adresse, $code_postal, $ville, $code_insee, $courriel, $tel, $url, $delai, $zone, $actif, $user_id, $id_offre);
	check_mysql_error($conn);
...
function update_criteres_offre($tab_villes, $tab_criteres, $user_id)
{
	global $conn;
...
function get_villes_by_competence_geo($competence, $id) // $row['competence_geo'], $row['id_competence_geo']
{
	global $conn;

	$villes = null;
	$query = 'SELECT `'.DB_PREFIX.'bsl__ville`.`code_insee`, MIN(`'.DB_PREFIX.'bsl__ville`.`code_postal`) AS cp, `'.DB_PREFIX.'bsl__ville`.`nom_ville` 
...
function get_offre_by_id($id)
{
	...
	$offre = null;
	$query = $sql = 'SELECT `id_offre`, `nom_offre`, `description_offre`, DATE_FORMAT(`debut_offre`, "%d/%m/%Y") AS date_debut, DATE_FORMAT(`fin_offre`, "%d/%m/%Y") AS date_fin, `id_sous_theme`, `adresse_offre`, `code_postal_offre`, `ville_offre`, `courriel_offre`, `telephone_offre`, `site_web_offre`, `delai_offre`, `zone_selection_villes`, `actif_offre`, `'.DB_PREFIX.'bsl_professionnel`.id_professionnel, `nom_pro`, `adresse_pro`, `code_postal_pro`, `ville_pro`, competence_geo, id_theme_pere, nom_departement, nom_region, nom_territoire, id_competence_geo 
