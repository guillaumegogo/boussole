...
if (isset($_POST["submit_meta"])) {
    if ($_POST["maj_id_territoire"]) {
        $req = 'UPDATE `'.DB_PREFIX.'bsl_territoire` SET `nom_territoire`="' . $_POST['libelle_territoire'] . '" WHERE `id_territoire`=' . $_POST['maj_id_territoire'];
        $result = mysqli_query($conn, $req);
        $id_territoire_choisi = $_POST['maj_id_territoire'];
    } else {
        $req = 'INSERT INTO `'.DB_PREFIX.'bsl_territoire`(`nom_territoire`) VALUES ("' . $_POST['libelle_territoire'] . '")';
        $result = mysqli_query($conn, $req);
...
if (isset($_POST["submit_villes"])) {
    //********* on efface
    $req3 = 'DELETE FROM `'.DB_PREFIX.'bsl_territoire_villes` WHERE `id_territoire` = ' . $_POST['maj_id_territoire'];
...
if (isset($_SESSION['territoire_id'])) {
    if ($_SESSION['territoire_id']) {
        ...
        $sql = 'SELECT `'.DB_PREFIX.'bsl__ville`.`code_insee`, `'.DB_PREFIX.'bsl__ville`.`code_postal`, `'.DB_PREFIX.'bsl__ville`.`nom_ville` 
