...
$sql = 'SELECT `id_utilisateur`, `'.DB_PREFIX.'bsl_utilisateur`.`id_statut`, `nom_utilisateur`, `email`, `libelle_statut`, `nom_pro`, `nom_territoire` '
...
if ($_SESSION['territoire_id']) {
    $sql .= ' AND (`'.DB_PREFIX.'bsl_utilisateur`.`id_statut`=2 AND `id_metier`= ' . $_SESSION['territoire_id'] . ') '
        . ' OR (`'.DB_PREFIX.'bsl_utilisateur`.`id_statut`=3 AND `'.DB_PREFIX.'bsl_professionnel`.`competence_geo`="territoire" AND `id_competence_geo`=' . $_SESSION['territoire_id'] . ')';
}
$sql .= ' ORDER BY `'.DB_PREFIX.'bsl_utilisateur`.`id_statut` ASC,`id_metier` ASC';
