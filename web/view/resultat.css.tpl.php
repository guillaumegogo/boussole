...
<?php foreach($offres as $t){ foreach($t as $offre){ ?>
#modal<?=$offre['id']?>{ display: none; position: fixed; top:0; right:0; bottom:0; left:0; background-color: rgba(0, 0, 0, 0.5); z-index: 1000; }
#modal<?=$offre['id']?>:target{ display: block; }
<?php } } ?>
	</style>
...
<?php 		} ?> 
				...
				<div class="resultat_offre">
					<!--<div class="coeur">&#9825;</div>-->
					<a href="#modal<?= (int) $offre["id"] ?>"><b><?php xecho($titre) ?></b></a>  <!-- onclick="afficheModal('<?= (int) $offre["id"] ?>');-->
				</div>
...
<div id="criteres" style="border:1px solid #29B297; text-align:center; margin:1em; padding:1em;">Rappel de mes informations : j'habite à <b><?php xecho($_SESSION['ville_habitee']) ?></b> et je souhaite <b><?php xecho(strtolower($_SESSION['besoin'])) ?></b>.<br/>Mes critères sont les suivants : <?php echo liste_criteres($_SESSION['critere'], ', '); ?>.
